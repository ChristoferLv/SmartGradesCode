...
public class UserAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        // System.out.println("Request Headers do 1: " +
        // Collections.list(request.getHeaderNames()));
        // print the endpoint receiving the request
        // System.out.println("Request URI: " + request.getRequestURI());

        // if its a preflight request, return 200

        //REMOVER ISSO QUANDO FINALIZADO !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        // String token = recoveryToken(request); // Recupera o token do cabeçalho
        ...
        // System.out.println("1 - Verifica se o endpoint requer autenticação antes de
        // processar a requisição");
        if (checkIfEndpointIsNotPublic(request)) {
            // System.out.println("2 - Recupera o token do cabeçalho Authorization da
            // requisição");
            String token = recoveryToken(request); // Recupera o token do cabeçalho Authorization da requisição
            if (token != null) {
                // System.out.println("3 - Obtém o assunto (neste caso, o nome de usuário) do
                // token");
                String subject = jwtTokenService.getSubjectFromToken(token); // Obtém o assunto (neste caso, o nome de
                                                                             // usuário) do token
                // System.out.println("4 - Busca o usuário pelo email (que é o assunto do
                // token)");
                User user = userRepository.findByEmail(subject).get(); // Busca o usuário pelo email (que é o assunto do
                                                                       // token)
                ...
                // System.out.println("7 - Define o objeto de autenticação no contexto de
    ...
    private String recoveryToken(HttpServletRequest request) {
        // System.out.println("10 - Recupera o token do cabeçalho Authorization da
        // requisição");
        // System.out.println("Request: " + request);
        // System.out.println("Request Headersdo 10: " +
        // Collections.list(request.getHeaderNames()));
    ...
    private boolean checkIfEndpointIsNotPublic(HttpServletRequest request) {
        // System.out.println("13 - Verifica se o endpoint requer autenticação antes de
        // processar a requisição");
